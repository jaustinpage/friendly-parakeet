...
``[options.entry_points]`` section in ``setup.cfg``::
    ...
    console_scripts =
         fibonacci = friendly_parakeet.skeleton:run
...
from friendly_parakeet import __version__
...
# `from friendly_parakeet.skeleton import fib`,
...
def parse_args(args: List[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--version",
        action="version",
        version="friendly-parakeet {ver}".format(ver=__version__),
    )
...
if __name__ == "__main__":
    ...
    #
    #     python -m friendly_parakeet.skeleton 42
    #
    run()
